...
def prepro_question_answer(imgs):
    ...
    for i, img in enumerate(imgs):
        ...
        for j, qa_pair in enumerate(img['qa_pairs']):
            question_txt = str(qa_pair['question']).lower().translate(string.punctuation).strip().split()
            img['processed_question_tokens'].append(question_txt)
            answer_txt = str(qa_pair['answer']).lower().translate(string.punctuation).strip().split()
            img['processed_answer_tokens'].append(answer_txt)
            processed_mc_tokens = []
            if 'multiple_choices' in qa_pair:
                for mc in qa_pair['multiple_choices']:
                    mc_txt = str(mc).lower().translate(string.punctuation).strip().split()
                    processed_mc_tokens.append(mc_txt)
            img['processed_mc_tokens'].append(processed_mc_tokens)
            if i < 10 and j == 0: print
            question_txt, answer_txt


def build_vocab(imgs, params):
    '''
      we build a word vocabulary from the questions and answers
      (only and not multiple answer choices) for all qa pairs for all images    .
...
def encode_question_answer(imgs, params, wtoi):
    ...
    # id of the QA pair
    label_id = np.zeros(M, dtype='uint32')
    ...
    counter = 1
    for i, img in enumerate(imgs):
        # n is number of questions/final_answers for 1 image
        ...
        for j, s in enumerate(img['final_questions']):
            question_label_length[question_counter] = min(max_question_length,
                                                          len(s))  # record the length of this sequence
        ...
        for j, s in enumerate(img['final_answers']):
            ...
            assert label_id[answer_counter] == img['qa_pairs'][j]['qa_id'], \
                'order of answers doesn\'t match order of questions'
            answer_counter += 1
            for k, w in enumerate(s):
    ...
    # converting to column vectors using concatenate
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_json', default='visual7w-toolkit/datasets/visual7w-telling/dataset.json',
    ...
    parser.add_argument('--num_multiple_choice', default=3, type=int,
                        help='number of multiple choices of each question.')
    parser.add_argument('--max_question_length', default=15, type=int,
    ...
    parser.add_argument('--max_answer_length', default=5, type=int,
                        help='max length of an answer, in number of words. answers longer than this get clipped.')
    parser.add_argument('--word_count_threshold', default=5, type=int,
                        help='only words that occur more than this number of times will be put in vocab')
    parser.add_argument('--image_dim', default=224, type=int,
                        help='dimension of image after rescale (224 is the input image dimension for VGGNet-16)')
    ...
    Lq, La, Lmc, label_start_ix, label_end_ix, question_label_length, answer_label_length, label_id = encode_question_answer(
        dataset['images'], params, wtoi)
